...
function Navbar() {
  ...
  return (
    <div className="container mx-auto flex justify-between items-center py-4 px-4 md:px-6">
      ...
      >
        <span
          className={`block w-6 h-0.5 bg-brand-primary transition-transform duration-300 ${mobileOpen ? "rotate-45 translate-y-2" : ""}`}
        />
        <span
          className={`block w-6 h-0.5 bg-brand-primary transition-opacity duration-300 ${mobileOpen ? "opacity-0" : ""}`}
        />
        <span
          className={`block w-6 h-0.5 bg-brand-primary transition-transform duration-300 ${mobileOpen ? "-rotate-45 -translate-y-2" : ""}`}
        />
      ...
      {mobileOpen && (
        <div className="absolute top-16 left-0 right-0 bg-cream-light shadow-lg z-50 md:hidden">
